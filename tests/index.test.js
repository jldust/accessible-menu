...
describe('AccessibleMenu', () => {
  ...
  describe('Constructor and Configuration', () => {
    it('should use default configuration when no config provided', () => {
      ...
      expect(menu.config.mobileControlId).toBe('nav-trigger');
    });
  ...
  describe('ARIA Controls Initialization', () => {
    it('should set ARIA attributes on menu buttons', () => {
      ...
      const button = menuContainer.querySelector(
        'button[data-plugin-id="btn1"]'
      );
  ...
  describe('Mobile Menu Controls', () => {
    ...
    it('should warn when mobile button ID is not found', () => {
      const consoleSpy = jest.spyOn(console, 'warn').mockImplementation();

      document.body.innerHTML = `
        <nav class="c-menu" data-mobile="#nonexistent">
          <ul class="menu"></ul>
        </nav>
      `;
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        'Mobile menu button with ID "nonexistent" not found'
      );
  ...
  describe('Mobile Menu Functionality', () => {
    ...
    it('should open mobile menu on button click', () => {
      ...
      expect(document.body.classList.contains('js-prevent-scroll')).toBe(true);
    });
    ...
    it('should close mobile menu on second button click', () => {
      ...
      mobileButton.click();
      expect(mobileButton.getAttribute('aria-expanded')).toBe('false');
      expect(document.body.classList.contains('js-prevent-scroll')).toBe(false);
    });
    ...
    it('should close all submenu buttons when closing mobile menu', () => {
      const subButton = menuContainer.querySelector(
        'button[data-plugin-id="btn2"]'
      );
